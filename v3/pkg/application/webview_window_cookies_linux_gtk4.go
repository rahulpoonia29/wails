...
extern void cookieRequestCallbackGTK4(unsigned int requestID, char **names, char **values, char **domains, char **paths, double *expires, bool *httpOnly, bool *secure, int count);
...
static void cookie_manager_get_cookies_finish(GObject *source, GAsyncResult *res, gpointer user_data) {
    ...
    if (error != NULL) {
        g_error_free(error);
        cookieRequestCallbackGTK4(requestID, NULL, NULL, NULL, NULL, NULL, NULL, NULL, -1);
        return;
    }
    ...
    result->count = count;
    if (count == 0) {
        cookieRequestCallbackGTK4(requestID, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0);
    ...
    cookieRequestCallbackGTK4(requestID, result->names, result->values, result->domains, result->paths,
...
//export cookieRequestCallbackGTK4
func cookieRequestCallbackGTK4(requestID C.uint, names **C.char, values **C.char, domains **C.char, paths **C.char, expires *C.double, httpOnly *C.bool, secure *C.bool, count C.int) {
