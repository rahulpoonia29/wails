...
// Replace import to add "net/http" correctly
// Let's use regex to find the first import block
file = file.replace(/import \(\s+"errors"/, `import (\n\t"net/http"\n\t"errors"`);
...
file = file.replace(/func \(w \*windowsWebviewWindow\) execJS\(js string\) {/m, `func (w *windowsWebviewWindow) getCookies(url string) []*http.Cookie {
	...
	w32.SendMessage(w.hwnd, w32.WM_APP + 100, uintptr(unsafe.Pointer(req)), 0)
